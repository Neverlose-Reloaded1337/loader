                ...
                _ => {
                    let existing_config =
                        match db::get_user_config(&state.db, user.id, *entry_id as i32).await {
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    "{prefix} failed to load config before local delete sync: {e}"
                                );
                                None
                            }
                        };
                    let deleted = db::delete_config(&state.db, user.id, *entry_id as i32).await;
                    if matches!(deleted, Ok(true)) {
                        if let Some(config) = existing_config {
                            local_storage::delete_config_file(&config.name);
                        }
                    }
                    deleted
                }
...
<<<<<<< HEAD
    let previous_config_name = if is_config_entry_type(entry_type) {
        match db::get_user_config(&state.db, user.id, entry_id).await {
            ...
            Err(e) => {
                tracing::warn!("{prefix} failed to load config before rename sync: {e}");
                None
            }
        }
    } else {
    ...
    if is_config_entry_type(entry_type) {
        if let Some(old_name) = previous_config_name.as_deref() {
            if old_name != new_name {
                local_storage::delete_config_file(old_name);
            }
        }

        match db::get_user_config(&state.db, user.id, entry_id).await {
            Ok(Some(config)) => local_storage::save_config_file(&config.name, &config.content),
            Ok(None) => {}
